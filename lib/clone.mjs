...
const cloneStructure = async (rootKey, targetProjectKey, cache = null) => {
  ...
  const createIssue = async (issue, targetProjectKey) => {
    ...
    console.info(`Cloning issue: ${issue.key} into project: ${targetProjectKey}`, {parentKey});
    ...
    console.info(`...cloned: ${issue.key} => ${newKey}`);
  ...
  const cloneSub = async oldKey => {
    ...
    if (processedKeys.has(oldKey)) {
      console.warn(`Skipping already processed issue: ${oldKey}`);
    ...
    console.log(`Cloning links for ${oldKey} => ${newKey}:`, issue.links);
    if (issue.links) {
      for (const { key: oldTargetKey, type, direction } of issue.links) {
        if (!isInSourceProject(oldTargetKey)) {
          console.log(`Skipping external link: ${oldTargetKey}`);
    ...
    console.log(`Cloning subtasks for ${oldKey} => ${newKey}:`, issue.subtasks);
    if (issue.subtasks) {
    ...
    for (const childKey of children) {
      console.log(`Cloning child issue: ${childKey}`);
  ...
  return clonedKey; // Return the cloned epic key
};
