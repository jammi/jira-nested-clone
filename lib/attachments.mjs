...
const handleAttachment = ({API_PATH, CACHE_DIR, auth}) => {
  ...
  const downloadAttachment = async attachment => {
    ...
    if (await fs.stat(filePath).catch(() => null)) {
      DEBUG_OUTPUT && console.log(`Attachment ${filePath} already downloaded, skipping.`);
      return filePath;
    }
    ...
    DEBUG_OUTPUT && console.log(`Attachment ${filePath} downloaded successfully.`);
    await delay(200);
  ...
  const uploadAttachmentToIssue = async (issueKey, filePath, attachment) => {
    ...
    DEBUG_OUTPUT && console.log(`Attachment ${filePath} uploaded successfully.`);
    await delay(200);
  };
  ...
  return async (sourceIssue, targetIssueKey) => {
    try {
      ...
      attachments.length && console.info(`Cloning ${attachments.length} attachments for issue: ${sourceIssueKey} -> ${targetIssueKey}`);
